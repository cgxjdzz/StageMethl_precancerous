library(readr)
survival=read_tsv('TCGA-COAD.survival.tsv')
temp=read_tsv('TCGA-COAD.methylation450.tsv')
...
write.csv(survival,file = 'colonic_survival_OSdata.csv')

temp=temp[,survival$sample]
write.csv(temp,file = 'colonic_survival_data.csv')


nana=survival[which(survival$OS==1),c('X_PATIENT','OS.time')]
nana=unique(nana)
