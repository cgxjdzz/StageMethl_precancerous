...
library(circlize)
pre_DMP=sum_DMP$Cirrhosis_to_Normal[which(abs(sum_DMP$Cirrhosis_to_Normal$logFC)>0.1),]
cancer_DMP=sum_DMP$Tumor_to_Normal[which(abs(sum_DMP$Tumor_to_Normal$logFC)>0.1),]
all_DMP=intersect(rownames(pre_DMP),rownames(cancer_DMP))
all_DMP=all_DMP[order(pre_DMP[all_DMP,]$P.Value)][c(1:150)]
load('0227_sum_norm.Rdata')
...
color=c("Normal" =  "#63BAAA", "Cirrhosis" = "#FCEAFF",'Tumor'='#5E0FDF')

pdf('0227liver_heatmap.pdf')
nana=HeatmapAnnotation(type=label, col = list(type=color),name='type')
p=Heatmap(draw_data,show_row_names = F,top_annotation = nana,col=colorRamp2(c(0, 0.5, 1), c("blue", "white", "red")),name = 'beta',show_column_names = F,cluster_columns = F,column_dend_reorder = F,row_dend_reorder = F,cluster_rows = F,row_split = c(rep(c("increase"), length(cancer_FC_down)),rep(c('decrease'),length(cancer_FC_up))))+
  rowAnnotation(df = logFCdf, col = list(pre=logFC_color,cancer=logFC_color), 
                width = unit(2, "cm"))
p
dev.off()
save(all_DMP,cancer_FC_down,cancer_FC_up,file='liver heatmapDMP.Rdata')
