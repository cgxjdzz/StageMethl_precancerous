...
}#data是要分割的一个列，对于每个元素都分割一次，which是取分割后的第几个
# ge.setwd() <- function(){
...
# p+stat_compare_means(comparisons = my_comparisons)+#??ͬ?????ıȽ?
...
ge.plot.pca <- function(data,type,title=""){
  #data就输入个数据集，type就是样品的名字，title是保存文件的名字
  library(ggbiplot)
  df10 <- data
  df10[is.na(df10)] <- 0
  names <-type
...
ge.groupdifference <- function(data,cut=0.9){
  #组内差异计算
  data=sort(data)
  l=data[(1-cut)/2*length(data)+1];
  u=data[(1-(1-cut)/2)*length(data)];
  result=data[data>l&data<u]
  mean=mean(result)
  return(c(log2(result[1]/mean),log2(result[length(result)]/mean)))
}

ge.disordersite <- function(site_var,mean_s){
  return((site_var-mean_s)/mean_s)
  
}

ge.disordercalculate <- function(data){
  site_var=as.numeric(apply(data,1,var))
  mean_s=mean(site_var)
  score_list=numeric()
  for( i in 1:length(site_var)){
  temp_score=ge.disordersite(site_var = site_var[i],mean_s )
  score_list[i]=temp_score
  }
  return(score_list)
  }

ge.disorderfinal <- function(data,m){
  return(sum(data)/m)
}